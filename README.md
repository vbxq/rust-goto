...
If you want to force LLVM to NOT merge (to experiment), disable tail merging:

```bash
RUSTFLAGS="-C llvm-args=-tail-merge-threshold=0" cargo rustc --release -- --emit=asm
```

((Or try with `opt-level = 2` and no LTO to see if LLVM merges more aggressively under weaker optimization))
